...
async def send_resume_feedback_email(to_email: str, analysis: Dict, job_role: str):
    ...
    # Run the actual sending in a separate thread to not block the event loop
    async def send_call():
        try:
            ...
            print(f"DEBUG: Initializing Brevo send to {to_email}...")
            api_instance = sib_api_v3_sdk.TransactionalEmailsApi(sib_api_v3_sdk.ApiClient(configuration))
            
            # The sender MUST be the email you verified in Brevo
            sender_email = settings.MAIL_FROM if settings.MAIL_FROM else "rajibmaharana8200@gmail.com"
            sender = {"name": "CareerSync", "email": sender_email}
            
            to = [{"email": to_email}]
            subject = f"Resume Analysis Report: {job_role}"
            ...
            print(f"DEBUG: SUCCESS! Brevo Message ID: {api_response.message_id}")
            
        except ApiException as e:
            print(f"DEBUG: Brevo API Error: {str(e)}")
        except Exception as e:
            print(f"DEBUG: Email task error: {str(e)}")

    # Use run_in_threadpool for the blocking SDK call
